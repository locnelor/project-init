...
import {
  CreateCardTemplateRequestOfPartner,
  DevelopmentConfigRequestOfPartner,
  FapiaoNotifyResultOfPartner,
  IssueFapiaoRequestOfPartner,
  RefundNotifyResultOfPartner,
  RefundParametersOfPartner,
  ReverseFapiaoRequestOfPartner,
  TradeOfPartner,
  TransactionOrderOfPartner
} from './types/wepay-partner';
...
export class WePayService {
   ...
   */
  public native(order: Omit<TransactionOrder, 'appid' | 'mchid'>) {
    const url = "v3/pay/transactions/native";
    // const nonceStr = createNonceStr();
    // const timestamp = Math.floor(Date.now() / 1000);

    // const signature = this.generateSignature('POST', url, timestamp, nonceStr, order);
    return axios.post<{ code_url: string }>(url, {
      ...order,
      appid: this.options.appId,
      mchid: this.options.mchid
    }, {
      // headers: this.generateHeader(this.options.mchid, nonceStr, timestamp, this.privateKey,)
    })
  }
  ...
  private generateHeader(options: GenerateHeaderOptions) {

    return {
      // 'Authorization':`WECHATPAY2-SHA256-RSA2048 mchid="${options.mchid || this.options.mchid}",nonce_str="${options.nonceStr || createNonceStr()}",timestamp="${options.timestamp || Math.floor(Date.now() / 1000)}",serial_no="${options.serialN}"`
    }
  }
   ...
   * @returns 
   * @link https://pay.weixin.qq.com/doc/v3/merchant/4012791879
   */
  public getNativeTransactionById(transaction_id: string) {
    const url = `/v3/pay/transactions/id/${transaction_id}?mchid=${this.options.mchid}`
   ...
   * @link
   */
  getNativeTransactionByIdOfPartner(out_trade_no: string) {
    const url = `/v3/pay/transactions/out-trade-no/${out_trade_no}?mchid=${this.options.mchid}`
    return axios.get<Trade>(url)
  }
  ...
  //  */
  // async getPlatformCertificates(serialNo: string, privateKey: Buffer | string, apiKey: string) {
  ...
  //   const ret = await axios.get<{ data: CertificateResult[] }>(url, { headers: this.generateHeader(this.options.mchid, nonceStr, timestamp, serialNo, signature) });
